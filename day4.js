...
function fourDigits(input, from, to) {
  ...
  if(integer < from) return false;
  if(integer > to) return false;
  return true;
}
...
class Passport {
  ...
  static fields = [ 'byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']
  ...
  isValid() {
    return Passport.fields.every(key => this.map.has(key));
  }

  isFullyValid() {
    return Passport.fields.every(key => this.map.has(key)) && 
    [...this.map.entries()].every(([key, value]) => Passport.fieldValidators[key](value)
    );
  }
}
